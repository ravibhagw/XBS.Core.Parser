...
namespace XBS.Core.Parser
{
    public class Parser
    {
        ...
        private string _loginFormData;

        public Parser(string targetSeason, int[] seasonStatsToPull, bool isEagerMode = true, bool loadTeamStats = true, string loginFormData = null)
        {
            ...
            this._loginFormData = loginFormData;

        }

        public List<Player> GetPlayers(bool eagerLoadStats = true)
        {
        ...
        {
            if(String.IsNullOrEmpty(_loginFormData))
            {
                return;
            }
            var req = WebRequest.CreateHttp("http://forum.xbox-sports.com/login.php?do=login");
            var formData = _loginFormData; // scrubbed for upload.  TODO: add configuration file.  If you have an active session/cookie you may not even need this.
